...
if (Test-Path -Path "grid.exe.bak") {
	...
	Invoke-WebRequest -Uri $steamless -OutFile ".\PatchingD\SLess.zip"
	Expand-Archive ".\PatchingD\SLess.zip" ".\PatchingD\" | Out-Null
	Write-Host ""
	Write-Host "Removing DRM..."
	Delay 3
	Start-Process -Wait ".\PatchingD\Steamless.CLI.exe" "grid.exe" | Out-Null
	Rename-Item -Path "grid.exe" "grid.bak"
	Rename-Item -Path "grid.exe.unpacked.exe" "grid.exe"
	Delay 3
	Write-Host ""
	Write-Host "DRM Removed"
	...
	Remove-Item -Path "grid.exe .Backup" -Force -ErrorAction SilentlyContinue
	Write-Host ""
